...
private class SecureHandler : RequestHandler<SecureQuery, String> {
    override suspend fun handle(mediator: Mediator, requestContext: RequestContext, request: SecureQuery) = "secure:${request.id}"
}

private class PublicHandler : RequestHandler<PublicQuery, String> {
    override suspend fun handle(mediator: Mediator, requestContext: RequestContext, request: PublicQuery) = "public:${request.id}"
}
...
class AuthorizationPipelineBehaviorTest {
    ...
    fun `does not apply to non-AuthenticatedRequest`() = runTest {
        ...
        val auth = AuthorizationPipelineBehavior(authorize = { _, _ -> authCalled = true; throw UnauthorizedException() })
