...
Build a small DSL on top of the existing API so each rule reads as one line and multiple errors accumulate before returning:
...
Now the validator becomes a flat list of rules — all are evaluated, all errors collected:
...
Because `FieldV` is a typed interface (implemented by your enum), use `when` to dispatch each error to the right UI state — no string comparison needed, and the compiler will warn you if you miss a case:
